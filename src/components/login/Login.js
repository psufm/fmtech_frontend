...
class Login extends Component {
  ...
  __getUserMailSaveCheck = () => {
    //메일값이 있으면 true, 아님 false
    return localStorage.getItem("mail") === null ? false : true; //메일값이 null이니? 그럼 false ! (false = 메일이 없다.)
  }; //메일이 있다 : true

  __getUserLogin = res => {
    // 로그인상태면? 연결 끊고, 그게 아니라면 로그인 로직 해주기 !
    this.__getUserMailSaveCheck()
      ? this.__disconnectUser(res)
      : this.__getUserAddressCheck(res);
  };
  ...
  __disconnectUser = res => {
    //상태값 설정. (로그아웃 로직 삭제 부분)
